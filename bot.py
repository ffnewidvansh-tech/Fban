...
def load_users():
    ...
    users = {
        ...
        "6181668524": {"id": 6181668524, "username": "DARE_VALID", "name": "DARE VALID", "joined": "2026-08-01T10:30:10.200151", "uses": 0, "unlimited": False, "banned": False}
    }
    save_users(users)
    return users
...
def get_admin_menu(user_id):
    ...
    markup.row(KeyboardButton(stylish_text("PRICE")), KeyboardButton(stylish_text("BAN CHECK PRICE")))
...
def format_ban_response(data):
    """Format ban info response"""
    if not data:
        return "❌ ᴀᴘɪ ᴇʀʀᴏʀ: ᴄᴏᴜʟᴅ ɴᴏᴛ ғᴇᴛᴄʜ ʙᴀɴ ɪɴғᴏ"
    ...
    status = "🚫 ʙᴀɴɴᴇᴅ" if is_banned else "✅ ᴀᴄᴛɪᴠᴇ"
    status_color = "🔴" if is_banned else "🟢"
    
    text = f"""
✅ ═══《 🔍 ʙᴀɴ ɪɴғᴏʀᴍᴀᴛɪᴏɴ 》═══ ✅

✅ 🆔 ᴀᴄᴄᴏᴜɴᴛ ɪᴅ: {account_id}
✅ 🏷️ ᴛʏᴘᴇ: {account_type}
✅ 👤 ɴɪᴄᴋɴᴀᴍᴇ: {nickname}
✅ 🌍 ʀᴇɢɪᴏɴ: {region}
✅ 📊 ʟᴇᴠᴇʟ: {level}
...
✅ 📌 ʙᴀɴ sᴛᴀᴛᴜs: {status_color} {status}
...
def check_ban_info_btn(message):
    try:
        user_id = message.from_user.id
        ...
        settings = load_settings()
        ban_check_price = settings.get("ban_check_price", 0)
        
        # Check if user has unlimited or free trial
        if ban_check_price > 0 and not user.get("unlimited", False):
            uses = user.get("uses", 0)
            if uses >= 1:
                _send_pe(message.chat.id, f"⚠️ ғʀᴇᴇ ᴛʀɪᴀʟ ᴜsᴇᴅ!\n💰 ᴘᴀʏ ʀs.{ban_check_price} ғᴏʀ ᴜɴʟɪᴍɪᴛᴇᴅ ʙᴀɴ ᴄʜᴇᴄᴋs")
                send_payment_qr(message.chat.id, "ban_check")
                return
        
        _send_pe(message.chat.id, f"🔍 sᴇɴᴅ ᴍᴇ ᴛʜᴇ ғʀᴇᴇ ғɪʀᴇ ᴜɪᴅ ᴛᴏ ᴄʜᴇᴄᴋ ʙᴀɴ ɪɴғᴏ:")
...
def process_ban_check(message):
    try:
        ...
        if data:
            ...
            response_text = format_ban_response(data)
            keyboard = [
                [make_green_button("ᴄʜᴇᴄᴋ ᴀɴᴏᴛʜᴇʀ", callback=f"check_another_{user_id}")],
                [make_blue_button("ɢᴇᴛ ᴜɴʟɪᴍɪᴛᴇᴅ", callback="get_unlimited")]
            ]
            markup = InlineKeyboardMarkup(keyboard)
            _send_pe(message.chat.id, response_text, reply_markup=markup)
        else:
            _send_pe(message.chat.id, f"❌ ғᴀɪʟᴇᴅ ᴛᴏ ғᴇᴛᴄʜ ʙᴀɴ ɪɴғᴏ! ᴘʟᴇᴀsᴇ ᴛʀʏ ᴀɢᴀɪɴ ʟᴀᴛᴇʀ.")
    except Exception as e:
        print(f"❌ Process ban check error: {e}")

@bot.callback_query_handler(func=lambda c: c.data and c.data.startswith("check_another_"))
def check_another_callback(call):
    try:
        ...
        _send_pe(call.message.chat.id, f"🔍 sᴇɴᴅ ᴛʜᴇ ғʀᴇᴇ ғɪʀᴇ ᴜɪᴅ ᴛᴏ ᴄʜᴇᴄᴋ:")
...
def ban_account_start(message):
    try:
        ...
        if not user.get("unlimited", False):
            uses = user.get("uses", 0)
            if uses >= 1:
                _send_pe(message.chat.id, f"⚠️ ғʀᴇᴇ ᴛʀɪᴀʟ ᴜsᴇᴅ!\n💰 ᴘᴀʏ ʀs.{load_settings().get('price', 29)} ғᴏʀ ᴜɴʟɪᴍɪᴛᴇᴅ")
...
def send_payment_qr(chat_id, payment_type="ban"):
    try:
        settings = load_settings()
        upi = settings.get("upi", "vanshx111@naviaxis")
        
        if payment_type == "ban_check":
            price = settings.get("ban_check_price", 0)
            if price == 0:
                _send_pe(chat_id, f"✅ ʙᴀɴ ᴄʜᴇᴄᴋ ɪs ғʀᴇᴇ!")
                return
        else:
            price = settings.get("price", 29)
...
"""
        ...
        keyboard = [
            [make_green_button("I HAVE PAID", callback=f"paid_{chat_id}_{payment_type}")],
...
def handle_paid(call):
    try:
        ...
        payment_type = parts[2] if len(parts) > 2 else "ban"
        ...
        bot.register_next_step_handler(call.message, receive_payment_screenshot, payment_type)
        bot.answer_callback_query(call.id)
    except Exception as e:
        print(f"❌ Paid callback error: {e}")

def receive_payment_screenshot(message, payment_type="ban"):
    try:
        user_id = message.from_user.id
        ...
        if message.photo:
            ...
            if str(user_id) in pending:
                ...
                pending[str(user_id)]["type"] = payment_type
                save_pending(pending)
...
"""
            keyboard = [
                [make_blue_button("✅ ᴀᴘᴘʀᴏᴠᴇ", callback=f"admin_approve_{user_id}_{payment_type}")],
...
def admin_approve_callback(call):
    try:
        ...
        parts = call.data.split("_")
        user_id = int(parts[2])
        payment_type = parts[3] if len(parts) > 3 else "ban"
        ...
        _send_pe(call.message.chat.id, f"✅ ᴜsᴇʀ {user_id} ᴀᴘᴘʀᴏᴠᴇᴅ ғᴏʀ {payment_type}!")
        
        try:
            bot.send_message(user_id, f"✅ ᴄᴏɴɢʀᴀᴛs! ʏᴏᴜ ɴᴏᴡ ʜᴀᴠᴇ ᴜɴʟɪᴍɪᴛᴇᴅ {payment_type} ᴀᴄᴄᴇss! 🎉")
        except:
            pass
...
def price_btn(message):
    _send_pe(message.chat.id, f"✅ 💰 ᴄᴜʀʀᴇɴᴛ: ʀs.{load_settings().get('price', 29)}\n✅ /price <ᴀᴍᴛ>")

@bot.message_handler(func=lambda m: m.text and stylish_text("BAN CHECK PRICE") in m.text)
def ban_check_price_btn(message):
    settings = load_settings()
    price = settings.get("ban_check_price", 0)
    if price == 0:
        _send_pe(message.chat.id, f"✅ 🔍 ʙᴀɴ ᴄʜᴇᴄᴋ ɪs ғʀᴇᴇ (ᴘʀɪᴄᴇ: 0)\n✅ /bancheckprice <ᴀᴍᴛ> ᴛᴏ sᴇᴛ ᴘʀɪᴄᴇ")
    else:
        _send_pe(message.chat.id, f"✅ 🔍 ʙᴀɴ ᴄʜᴇᴄᴋ ᴘʀɪᴄᴇ: ʀs.{price}\n✅ /bancheckprice <ᴀᴍᴛ> ᴛᴏ ᴄʜᴀɴɢᴇ")
...
✅ /checkall - ᴄʜᴇᴄᴋ ᴀʟʟ
✅ /totaladmins - ᴀᴅᴍɪɴs
✅ /price <ᴀᴍᴛ> - ᴄʜᴀɴɢᴇ ʙᴀɴ ᴘʀɪᴄᴇ
✅ /bancheckprice <ᴀᴍᴛ> - ᴄʜᴀɴɢᴇ ʙᴀɴ ᴄʜᴇᴄᴋ ᴘʀɪᴄᴇ
✅ /upi <ᴜᴘɪ> - ᴄʜᴀɴɢᴇ ᴜᴘɪ
...
def price_cmd(message):
    ...
    if len(parts) < 2:
        settings = load_settings()
        _send_pe(message.chat.id, f"✅ 💰 ᴄᴜʀʀᴇɴᴛ ʙᴀɴ ᴘʀɪᴄᴇ: ʀs.{settings.get('price', 29)}\n✅ /price <ᴀᴍᴛ>")
        return
    try:
        price = int(parts[1])
        settings = load_settings()
...
@bot.message_handler(commands=['bancheckprice'])
def bancheckprice_cmd(message):
    if not is_admin(message.from_user.id):
        _send_pe(message.chat.id, f"❌ ᴜɴᴀᴜᴛʜᴏʀɪᴢᴇᴅ!")
        return
    parts = message.text.split()
    if len(parts) < 2:
        settings = load_settings()
        price = settings.get("ban_check_price", 0)
        _send_pe(message.chat.id, f"✅ 🔍 ᴄᴜʀʀᴇɴᴛ ʙᴀɴ ᴄʜᴇᴄᴋ ᴘʀɪᴄᴇ: ʀs.{price}\n✅ /bancheckprice <ᴀᴍᴛ>")
        return
    try:
        price = int(parts[1])
        settings = load_settings()
...
✅ 💳 ʙᴀɴ ᴘʀɪᴄᴇ: ʀs.{settings.get('price', 29)}
✅ 🔍 ʙᴀɴ ᴄʜᴇᴄᴋ: ʀs.{settings.get('ban_check_price', 0)}
